...
class Solution { 
    ...
    public void solve(int depth, int[] nowHints, int money){
        ...
        solve(depth+1, nowHints.clone(), money+cost[depth][hintCnt]);
        
        //힌트 사는 경우
        int[] next = nowHints.clone();
        int hintCost = hint[depth][0]; //힌트비용
        for(int i=1; i<hint[depth].length; i++){
            int t = hint[depth][i];
            next[t-1]++; 
        }
        int hintCnt2 = Math.min(next[depth], n-1); //가지고 있는 힌트 개수
        solve(depth+1, next, money+hintCost+cost[depth][hintCnt2]);
